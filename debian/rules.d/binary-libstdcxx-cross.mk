...
p_lib_native = libstdc++$(CXX_SONAME)
...
d_lib	= debian/$(p_lib)
d_lib_native = debian/$(p_lib_native)
...
$(binary_stamp)-libstdcxx: $(install_stamp)
	...
	rm -rf $(d_lib_native)
	dh_installdirs -p$(p_lib_native) $(dirs_lib_native)
	find $(d_lib) -name 'libstdc++*so*' -type f -exec \
		cp -a "{}" $(d_lib_native)/$(dirs_lib_native) \;
	find $(d_lib) -name 'libstdc++*so*' -type l -exec \
		cp -a "{}" $(d_lib_native)/$(dirs_lib_native) \;
	...
	ARCH=$(DEB_TARGET_ARCH) MAKEFLAGS="CC=something" dh_shlibdeps -p$(p_lib_native)
	DEB_HOST_ARCH=$(DEB_TARGET_ARCH) \
		dh_gencontrol -p$(p_lib_native) -u-v$(DEB_VERSION)

	dh_md5sums -p$(p_lib)
	dh_builddeb -p$(p_lib)

	dh_md5sums -p$(p_lib_native)
	dh_builddeb -p$(p_lib_native)
...
ifdef GCC_LIBC_TARGET
 ...
 ifeq ($(biarch),yes)
	mv $(d)/$(PF)/$(subst -gnu,-$(GCC_LIBC_TARGET),$(DEB_TARGET_GNU_TYPE))/lib64/lib*c++*.a $(d)/$(gcc_lib_dir)/64/.
