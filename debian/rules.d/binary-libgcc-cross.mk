...
p_lgcc_native	= libgcc$(GCC_SONAME)
d_lgcc_native	= debian/$(p_lgcc_native)
...
files_l64gcc = \
	$(DEB_TARGET_GNU_TYPE)/lib64/libgcc_s.so.$(GCC_SONAME)
endif
...
$(binary_stamp)-libgcc: $(install_dependencies)
	...
	rm -rf $(d_lgcc) $(d_lgcc_native)
	dh_installdirs -p$(p_lgcc) \
		$(docdir)/$(p_lgcc) \
		$(PF)/$(DEB_TARGET_GNU_TYPE)/lib
	dh_installdirs -p$(p_lgcc_native) \
		$(docdir)/$(p_lgcc_native) \
		lib

ifeq ($(with_shared_libgcc),yes)
	DH_COMPAT=2 dh_movefiles -p$(p_lgcc) $(files_lgcc)
	cp $(d_lgcc)/$(files_lgcc) $(d_lgcc_native)/lib
endif
