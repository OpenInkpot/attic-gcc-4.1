...
ifeq ($(biarch32),yes)
  ...
  ifeq ($(distribution),Debian)
	dh_link -p$(p_gcc) \
	  /usr/lib32/libgcc_s.so.$(GCC_SONAME) /$(gcc_lib_dir)/libgcc_s_32.so \
	  /usr/lib32/libgcc_s.so.$(GCC_SONAME) /$(gcc_lib_dir)/32/libgcc_s_32.so \
	  /usr/lib32/libgcc_s.so.$(GCC_SONAME) /$(gcc_lib_dir)/32/libgcc_s.so
  else
	dh_link -p$(p_gcc) \
